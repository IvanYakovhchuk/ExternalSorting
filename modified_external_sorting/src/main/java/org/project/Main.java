...
public class Main {
    static final int MAX_SERIES_SIZE = 8_000_000;

    public static void main(String[] args) throws IOException {
        File sourceFile = generateInputFile(200_000_000);
        ...
        File fileToDelete = new File("mod_input.txt");
        fileToDelete.delete();
    }
    ...
    public static void polyphaseMergeSort(File sourceFile, List<File> auxFiles) throws IOException {
        File newSourceFile = increaseRunsLength(sourceFile);
        distributeRuns(newSourceFile, auxFiles);
    ...
    public static void readAndWrite(BufferedReader br, BufferedWriter bw, int numOfRuns, List<Integer> currentRun, boolean lastCall) throws IOException {
        ...
        while (counter < numOfRuns && (line = br.readLine()) != null) {
            ...
            if ((currentRun.isEmpty() || currentRun.getLast() <= num) && currentRun.size() < MAX_SERIES_SIZE + 1) {
                currentRun.add(num);
            }
            else {
    ...
    private static File increaseRunsLength(File file) throws IOException {
        String line;
        int num;
        List<Integer> setAsRun = new ArrayList<>(MAX_SERIES_SIZE); // Використовуємо ініціалізацію
        File tempFile = new File("mod_input.txt");

        // Зчитуємо дані і обробляємо блоками
        try (BufferedReader br = new BufferedReader(new FileReader(file))) {
            while ((line = br.readLine()) != null) {
                num = Integer.parseInt(line.trim());
                setAsRun.add(num);

                if (setAsRun.size() == MAX_SERIES_SIZE) {
                    Collections.sort(setAsRun);
                    writeRunsToFile(setAsRun, tempFile);
                    setAsRun.clear();
                }
            }

            // Обробка залишкових чисел
            if (!setAsRun.isEmpty()) {
                Collections.sort(setAsRun);
                writeRunsToFile(setAsRun, tempFile);
            }
    ...
    // Метод для запису відсортованих даних у тимчасовий файл
    private static void writeRunsToFile(List<Integer> setAsRun, File tempFile) throws IOException {
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(tempFile, true))) {
            for (int run : setAsRun) {
                bw.write(run + "\n");
            }
        }
    }
